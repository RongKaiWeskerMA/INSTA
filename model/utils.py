...
def c_acc(logits, labels_test):
    _, preds = torch.max(logits, 1)
    acc = (torch.sum(preds == labels_test)).type(torch.cuda.FloatTensor) / labels_test.size(0)
...
def get_command_line_parser():
    ...
    parser.add_argument('--model_class', type=str, default='INSTA_PorotNet',
                        choices=['INSTA_ProtoNet', 'ProtoNet'])
    parser.add_argument('--use_euclidean', action='store_true', default=False)
    parser.add_argument('--use_AdamW', action='store_true', default=False)
    parser.add_argument('--backbone_class', type=str, default='Res12',
                        choices=['Res12', 'Res18'])
    parser.add_argument('--dataset', type=str, default='MiniImageNet',
                        choices=['MiniImageNet', 'TieredImageNet', 'CUB', 'FC100'])
    ...
    parser.add_argument('--balance_1', type=float, default=0)
    parser.add_argument('--balance_2', type=float, default=0)
    ...
    parser.add_argument('--baseline', type=str, default='y')
